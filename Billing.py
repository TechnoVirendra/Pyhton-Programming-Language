...
cursor=mydb.cursor()
sql="create database Billing;"
sql1="use billing;"
sql2="create table shop(C_Id int,C_Name Varchar(15),Item_Code int,I_Name varchar(15),Price varchar(8),Qnt varchar(4),Disc varchar(8),Amount varchar(12));"
cursor.execute(sql)
cursor.execute(sql1)
cursor.execute(sql2)
...
def strinsert():
    ...
    L.append(Qnt)
    S=(L)
    sq="insert into shop(C_Id,C_Name,Item_Code,I_Name,Price,Qnt,Disc,Amount)values(%s,%s,%s,%s,%s,%s,10*Price*Qnt/100,Price*Qnt-Disc);"
    cursor.execute(sq,S)
    mydb.commit()
# View Data.
def StuView():
    ...
    if choice==1:
        ...
        r1=(z,)
        sq="select * from shop where C_Id=%s;"
        cursor.execute(sq,r1)
    elif choice==2:
        z=input("Enter Customer Name : ")
        r1=(z,)
        sq="select * from shop where C_Name=%s;"
        cursor.execute(sq,r1)
    elif choice==3:
        sq="select * from shop;"
        cursor.execute(sq)
    ...
    print("C_Name,C_Id,Item_Code,I_Name,Price,Qnt,Disc,Amount")
    for x in res:
        print(x)
# Remove Items
def removeStu():
    ...
    r1=(Item_Name)
    sq="delete from shop where I_Name='%s';"
    cursor.execute(sq,r1)
    mydb.commit()
# Menu Set.
def Menuset():
